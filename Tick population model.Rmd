...
# f is the probability of taking a blood meal 
  ...
  u = params[11] # natural mortality (not finding a blood meal)
  d = params[12] # probability of adult finding a deer and successfully mating
  h = params[13] # number of eggs hatching successfully into larvae
  
  # differential equations to track the change in tick life stages over time
  dTldt = Tl*(f1*C1 - f2*C2 - f3*C3) + d*h*Ta - u*Tl 
  dTndt = m*Tl*(f1*C1 + f2*C2 + f3*C3) - Tn*(f4*C1 - f5*C2 - f6*C3) - u*Tn
...
parameters = c(f1, f2, f3, f4, f5, f6, C1, C2, C3, m, u, d, h)
