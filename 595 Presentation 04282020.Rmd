...
eggstoadults <- function(t, x, params) {
  ...
  # LARVAE
  # 3 classes from questing to feeding
  Lqi = x[1] # infected stays infected
  ...
  #NYMPHS
  Nqi = x[8] # infected stays infected
  Nqs.a = x[9] # susceptible becomes infected
  ...
  #ADULTS
  Aqi = x[15] # infected stays infected
  Aqs.a = x[16] # susceptible becomes infected
  Aqs.b = x[17] # susceptible stays susceptible
  ...
  phi = params[6] # dropping off rate
  tau = params[7] # molting rate
  delta = params[8] # birth rate
  tot = params[9] # total ovarial transmission (tot)
  tot_prime = params[10] # no tot
  rho = params[11] # filial infection prevalence (fip)
  rho_prime = params[12] # no fip
  kappa = params[13] # survival from egg to larva to next stage (need to make specific)
  
  # S + I = N host in system
  
  # questing to feeding
  dLqidt = psi - omega*Lqi - lambda*Lqi + rho*Gv*kappa # infected stays infected
  
  # this line needs help, 
  #which do I assign death to? Do I combine both routes for si here?
  dLqs.adt = psi - omega*Lqs.a - lambda*Lqs.a*alpha - omega*Lqs.a -lambda*Xqs.a*sigma + rho_prime*Gv*kappa # susceptible becomes infected
  # if sigma/alpha 0, selects 1 route of transmission
  
  dLqs.bdt = psi - omega*Lqs.b - lambda*Lqs.b + kappa*Gu + rho_prime*Gv*kappa # susceptible stays susceptible
  
  # feeding to engorged
  dLfidt = lambda*Lqi + lambda*Lqs.a*alpha + lambda*Lqs.a*sigma - omega*Lfi - phi*Lfi
  dLfsdt = lambda*Lqs.b - omega*Lfs - phi*Lfs
  
  # engorged to molted to next X
  dLeidt = phi*Lfi - omega*Lei - tau*Lei
  dLesdt = phi*Lfs - omega*Les - tau*Les
  
  ## NYMPHS
  
   # questing to feeding
  dNqidt = - omega*Nqi - lambda*Nqi + tau*Lei # infected stays infected
